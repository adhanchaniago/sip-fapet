...
function submit(x) {
    if (x == 'add') {
        ...
        document.getElementById("viewfoto").src = '<?=$base_url;?>uploads/mahasiswa/default.jpg';
        // $('#prodi_other').hide();
        $('#remove_prodi').remove();
        $('#btn-ubah').hide();
        $('#btn-add').show();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var link = '<?=$base_url;?>uploads/mahasiswa/' + img
                ...
                var prodi = ["Nutrisi Teknologi Pakan Ternak", "Peternakan", "Budidaya Ternak",
                    "Kesehatan Hewan"
                ];
                var is_prodi = prodi.indexOf(data.program_studi);
                if (is_prodi >= 0) {
                    $('[name="program_studi"]').val(data.program_studi);
                } else {
                    $('[name="program_studi"]').val("Lainnya");
                    otherProdi();
                    $('[name="other_program_studi"]').val(data.program_studi);
                }
            }
        });
    }
...
function ubah_gambar(x) {
    $.ajax({
        ...
        success: function(data) {
            ...
            var html = '<img src="<?=$base_url;?>uploads/mahasiswa/' + img +
...
function otherProdi() {
    var selectBox = document.getElementById("program_studi");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    // alert(selectedValue);
    if (selectedValue === 'Lainnya') {
        var html = `<div class="form-group" id="remove_prodi">
            <label class="control-label">Program Studi<span style="color:red;">*</span></label>
            <input type="text" class="form-control" name="other_program_studi"
                placeholder="ex: Nutrisi Teknologi Pakan Ternak" required>
        </div>`;
        $('#prodi_other').html(html);
    } else {
        $('#remove_prodi').remove();
    }
}
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="#modal_image" data-toggle="modal"
                                        ...
                                        <img src="<?=$base_url;?>uploads/mahasiswa/<?=($row['foto']!='')?$row['foto']:'default.jpg';?>"
                                            alt="<?=$row['foto'];?>"
                                ...
                                <td>
                                    <a href="<?=$base_url;?>proses/barcode_mhs.php?param=<?=$row['idmahasiswa'];?>"
                                    ...
                                    <a href="<?=$base_url;?>proses/mhs.php?id=<?=$row['idmahasiswa'];?>"
...
<div class="modal" id="modal_add" tabindex="-1" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?=$base_url;?>proses/mhs.php" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="col-md-12" style="margin-bottom:15px;">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="nama_lengkap"
                                        placeholder="ex: Nunung Mappa Windah" required>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3" style="margin-bottom:15px;">
                                <div class="form-group">
                                    <label class="control-label">Jenjang Pendidikan<span
                            ...
                            <div class="col-md-6" style="margin-bottom:15px;">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="program_studi" id="program_studi"
                                        onchange="otherProdi()" required>
                                        <?php foreach (prodi() as $o) :?>
                                        <option value="<?=$o;?>"><?=$o;?></option>
                                        <?php endforeach;?>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12" style="margin-bottom:15px;" id="prodi_other">
                                <!-- <div class="form-group">
                                    <label class="control-label">Program Studi<span style="color:red;">*</span></label>
                                    <input type="text" class="form-control" name="other_program_studi"
                                        placeholder="ex: Nutrisi Teknologi Pakan Ternak" required>
